		...
		if (nimmer .eq. 1) then
			...
			if (nparticle .ne. 1) then
				...
				enddo
                        ...
                        read(1,*) xatt
                        xatt=pi/180.0d0*xatt
                        read(1,*) nfcent,nfcent4
 ...
 336		enddo			
		...
		if (nimmer .eq. 1) then
			...
			if (neatemp(nn1,nis(2)) .eq. 0) then
						neats(1)=neatemp(nn1,1)
						neats(2)=neatemp(nn1,2)
						neats(3)=neatemp(nn1,3)
						neats(4)=neatemp(nn1,4)
						neats(5)=neatemp(nn1,5)
						neats(6)=neatemp(nn1,6)
						neats(7)=neatemp(nn1,7)
						neatemp(nn1,1)=neats(1)
						neatemp(nn1,2)=neats(2)
						neatemp(nn1,3)=neats(3)
						neatemp(nn1,4)=neats(1)
						neatemp(nn1,5)=neats(4)
						neatemp(nn1,6)=neats(5)
						neatemp(nn1,7)=neats(6)
						neatemp(nn1,8)=neats(1)
						neatemp(nn1,9)=neats(7)
				endif
				do k=1,nis(2)
     ...
     $					ntx3,ntx4,ntx5
				if (neaptemp(nn1,4) .eq. 0) then
						neaptemp(nn1,4)=neaptemp(nn1,3)
					endif
				do k=1,4
			...
			do k=1,nndaim
				j=nndfsia2+k
                  jcent=nfcent4+nndfsia2
                  xc1=coorptemp(j,1)-coorptemp(jcent,1)
                  xc2=coorptemp(j,2)-coorptemp(jcent,2)
                  coorptemp(j,1)=coorptemp(jcent,1)+dcos(xatt)*xc1
                  coorptemp(j,1)=coorptemp(j,1)+dsin(xatt)*xc2
                  coorptemp(j,2)=coorptemp(jcent,2)-dsin(xatt)*xc1
                  coorptemp(j,2)=coorptemp(j,2)+dcos(xatt)*xc2
              enddo												
			if (nparticle .ne. 1) then
				do ip=1,npart
					if (ip .eq. 1) then
	...
	enddo
      ...
      read(1,*) numskew,numgb,intnum,numct,nteth
      do j=1,nteth
         read(1,*) ntnod(j),xnt(1,j),xnt(2,j)
         ntnod(j)=ntnod(j)+nnd-nndim*npart
	write(*,*) j,ntnod(j),coor(ntnod(j),1),coor(ntnod(j),2)
      enddo
      do j=1,numgb
         read(1,*) ndirgb(j)
         read(1,*) numdir(j)
...
c		do j=1,npr
c			ntemm=nprint(j)
c	write(*,*) npr,j,nprint(j),mapnea(ntemm)
c			nprint(j)=mapnea(ntemm)
c		enddo
	endif
	do i=1,intnum
